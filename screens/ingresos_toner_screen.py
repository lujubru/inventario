    ...
    MDBoxLayout:
        ...
        MDBoxLayout:
            ...
            ScrollView:
                MDList:
                    MenuButton:
                        text: "Registrar Ingreso de Toner"
                        icon: "plus-circle"
        ...
        MDBoxLayout:
            ...
            MDTopAppBar:
                title: "Registrar Ingreso de Toner"
                elevation: 4
            ...
            ScrollView:
                MDBoxLayout:
                    ...
                    MDTextField:
                        ...
                        on_text_validate: root.on_buscar_toner_enter()
...
class ConfirmacionPopup(Popup):
    def __init__(self, confirmar_callback, **kwargs):
        ...
        self.size_hint = (0.6, 0.4)
        content = MDBoxLayout(orientation='vertical', padding=10, spacing=10)
        content.add_widget(MDLabel(text='¿Está seguro de que desea registrar este ingreso de toner?'))
...
class IngresosTonerScreen(MDScreen):
    toners = ListProperty([])
    ...
    def check_permissions(self, dt):
        ...
        if not permisos or 'ingresos_toner' not in permisos[0][0].split(','):
            error_popup = ErrorPopup(message='No tienes permiso para acceder a la pantalla de ingresos de toner')
    ...
    def registrar_ingreso(self):
        ...
        if not toner or not cantidad:
            self.ids.message.text = 'Por favor, complete los campos de toner y cantidad'
            return
        try:
        ...
        execute_query("INSERT INTO movimientos_toner (toner_id, tipo, cantidad, fecha, observaciones) VALUES (?, ?, ?, ?, ?)",
                      (toner_id, 'ingreso', cantidad, datetime.now(), observaciones))
        self.ids.message.text = 'Ingreso de toner registrado correctamente'
        self.clear_fields()

    def on_buscar_toner_enter(self):
