...
class ConfirmacionPopup(Popup):
    def __init__(self, confirmar_callback, **kwargs):
        ...
        self.size_hint = (0.6, 0.4)
        content = MDBoxLayout(orientation='vertical', padding=10, spacing=10)
        content.add_widget(MDLabel(text='¿Está seguro de que desea registrar este egreso?'))
...
class EgresosScreen(MDScreen):
    ...
    def check_permissions(self, dt):
        ...
        if not permisos or 'egresos' not in permisos[0][0].split(','):
            error_popup = ErrorPopup(message='No tienes permiso para acceder a la pantalla de egresos')
    ...
    def registrar_egreso(self):
        ...
        if not articulo or not cantidad or not observaciones:
        ...
        stock_actual = fetch_data("SELECT cantidad FROM articulos WHERE id = ?", (articulo_id,))[0][0]
        if stock_actual >= cantidad:
            execute_query("UPDATE articulos SET cantidad = cantidad - ? WHERE id = ?", (cantidad, articulo_id))
            usuario_actual = self.app.usuario_actual
            execute_query("INSERT INTO movimientos (articulo_id, tipo, cantidad, fecha, observaciones, usuario) VALUES (?, ?, ?, ?, ?, ?)",
                          (articulo_id, 'egreso', cantidad, datetime.now(), observaciones, usuario_actual))
            self.ids.message.text = 'Egreso registrado correctamente'
            self.clear_fields()
        else:
            self.ids.message.text = 'Stock insuficiente'

    def clear_fields(self):
        self.ids.buscar_articulo.text = ''
